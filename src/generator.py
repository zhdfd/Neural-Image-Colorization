...
class Generator(net.Net):
    ...
    def build(self, x, nf=64, oc=2):
        ...
        with tf.variable_scope('generator'):
            ...
            self.conv2d = self.__residual_layer(self.conv1d, self.conv6e, nf * 8, drop=True, name='conv2d')
            self.conv3d = self.__residual_layer(self.conv2d, self.conv5e, nf * 8, drop=True, name='conv3d')
            self.conv4d = self.__residual_layer(self.conv3d, self.conv4e, nf * 8, name='conv4d')
            self.conv5d = self.__residual_layer(self.conv4d, self.conv3e, nf * 4, name='conv5d')
            self.conv6d = self.__residual_layer(self.conv5d, self.conv2e, nf * 2, name='conv6d')
            self.conv7d = self.__residual_layer(self.conv6d, self.conv1e, nf, name='conv7d')
