...
class Discriminator(net.Net):
    ...
    def predict(self, y, x, noise=None, reuse_scope=False):
        ...
        with tf.variable_scope('discriminator'):
            ...
            conv0 = self.conv_layer(xy, 64, norm=False, pad='SAME', stride=self.stride1, name='conv0')

            conv1 = self.conv_layer(conv0, 128, pad='SAME', stride=self.stride1, name='conv1')
            conv2 = self.conv_layer(conv1, 256, pad='SAME', stride=self.stride1, name='conv2')
            conv3 = self.conv_layer(conv2, 512, pad='SAME', stride=self.stride1, name='conv3')
            conv4 = self.conv_layer(conv3, 512, pad='SAME', stride=self.stride1, name='conv4')

            conv5 = self.conv_layer(conv4, 512, pad='SAME', stride=self.stride2, name='conv5')
